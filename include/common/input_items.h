...
class InputAnalog {
public:
  InputAnalog(AnalogSetting const *settings)
      : kDirection{settings->direction}, kReverse{settings->reverse},
        kIndex{settings->index} {}
  virtual void Update(uint8_t analog) = 0;

  int32_t Index() const { return kIndex; }
...
public:
  InputStick(StickSetting const *settings, float x_divisor, float y_divisor)
      : kRangeX{settings->x_range / x_divisor}, kRangeY{settings->y_range /
                                                        y_divisor},
        kIndexX{settings->x_index}, kIndexY{settings->y_index} {}
  virtual void Update(int8_t x, int8_t y) = 0;
  virtual ~InputStick() {}

  int32_t IndexX() const { return kIndexX; }

  int32_t IndexY() const { return kIndexY; }
