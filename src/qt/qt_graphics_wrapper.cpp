...
QTGraphicsWrapper::QTGraphicsWrapper(QGraphicsView *graphics_view)
    : graphics_view_{graphics_view}, scene_{nullptr}, dispatch_{nullptr},
      skin_directory_{}, bg_width_{0}, bg_height_{0} {}
...
void QTGraphicsWrapper::Update() { graphics_view_->update(); }
...
bool QTGraphicsWrapper::SetupScene(std::string_view skin_directory,
  ...
  if (xml_file.open(QIODevice::ReadOnly)) {
        ...
        if (xml.name().compare("background") == 0) {
          ...
          if (xml.name().compare("button") == 0) {
            ...
            if (!viewer->AssignStick(
                    xml.attributes().value("xname").toString().toStdString(),
                    xml.attributes().value("yname").toString().toStdString(),
                    stick)) {
              delete stick;
              return false;
            }
    ...
    std::cerr << "Error opening " << skin_directory_.toStdString() << "skin.xml"
              << std::endl;
    return false;
  }
...
int32_t QTGraphicsWrapper::GetHeight() const { return bg_height_; }

int32_t QTGraphicsWrapper::GetWidth() const { return bg_width_; }
...
QTGraphicsWrapper::CreateStick(QXmlStreamAttributes const &attributes) {
  ...
  return new QtInputStick(image, attributes.value("x").toInt(),
                          ...
                          attributes.value("yrange").toInt(), 128.f, 128.f);
}
