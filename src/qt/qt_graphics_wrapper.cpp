...
void QTGraphicsWrapper::StartDispatchThread(std::function<void()> const& tick_callback) {
    dispatch_ = new QTimer(this);
    connect(dispatch_, &QTimer::timeout, this, [tick_callback](){tick_callback();});
    dispatch_->start();
}
...
bool QTGraphicsWrapper::SetupScene(std::string_view skin_directory, Viewer* viewer) {
    ...
    if (xml_file.open(QIODevice::ReadOnly)) {
        ...
        while (!xml.isEndDocument()) {
            ...
            if (xml.isStartElement())
            {
                if (xml.name().compare("background") == 0) {
                    background_init = SetBackground(xml.attributes());
                    if (!background_init) {
                        return false;
                    }
                }
                    ...
                    if (xml.name().compare("button") == 0) {
                        QtInputButton* button = CreateButton(xml.attributes());
                        if (button == nullptr) {
                            return false;
                        }

                        if (!viewer->AssignButton(xml.attributes().value("name").toString().toStdString(), button)) {
                            delete button;
                            return false;
                        }
                        ...
                        QtInputStick* stick = CreateStick(xml.attributes());
                        if (stick == nullptr){
                            return false;
                        }

                        if (!viewer->AssignStick(xml.attributes().value("xname").toString().toStdString(), xml.attributes().value("yname").toString().toStdString(), stick)) {
                            delete stick;
                            return false;
                        }
        ...
        std::cerr << "Error opening " << skin_directory_.toStdString() << "skin.xml" << std::endl;
        return false;
    }
...
QtInputButton* QTGraphicsWrapper::CreateButton(QXmlStreamAttributes const& attributes) {
...
QtInputStick* QTGraphicsWrapper::CreateStick(QXmlStreamAttributes const& attributes) {
    QPixmap image{CreateImage(attributes)};
    ...
    return new QtInputStick(
            image,
            attributes.value("x").toInt(),
            attributes.value("y").toInt(),
            attributes.value("xrange").toInt(),
            attributes.value("yrange").toInt(),
            128.f,
            128.f
        );
}
