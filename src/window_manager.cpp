...
void WindowManager::OpenViewer(int32_t com_index) {
    viewer_window_ = std::make_unique<ViewerWindow>(nullptr, [this](){OpenMain();}, com_index);
    if (!viewer_window_->Valid()) {
        viewer_window_ = nullptr;
        return;
    }
    viewer_window_->setWindowFlags(
        viewer_window_->windowFlags() &(~Qt::WindowMaximizeButtonHint));
    viewer_window_->show();
