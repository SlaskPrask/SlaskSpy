...
std::unordered_map<ViewerType, std::vector<SkinData>>
SkinSettings::FetchSkins(std::string const &skins_directory) {
...
std::tuple<std::vector<ViewerType>, SkinData>
  ...
  if (xml_file.is_open()) {
    ...
    while (std::getline(xml_file, line)) {
      ...
      if (line.find("<skin") == std::string::npos) {
        Logger::Error("skin_settings: Skin %s is not a valid skin.xml",
                      skin_xml_path.c_str());
    ...
    while ((pos_end = type_string.find(';', pos_start)) != std::string::npos) {
      ...
      if (type_value == ViewerType::kNull) {
        Logger::Warn("skin_settings: Skin type %s is not implemented. Skin: %s",
...
std::string_view SkinSettings::GetSkinPath() const { return skin_path_; }

SkinSettings::SkinSettings(std::string_view skin_directory, ViewerType type)
    : valid_{false}, skin_path_{skin_directory}, type_{type}, buttons_{},
      sticks_{}, background_{} {
  ...
  if (xml_file.is_open()) {
    ...
    while (std::getline(xml_file, line)) {
      ...
      } else if (element_line.find("<stick") != std::string::npos) {
      ...
      } else if (element_line.find("<background") != std::string::npos) {
        if (!CreateBackgroundSetting(element_line)) {
          break;
        } else {
...
bool SkinSettings::CreateAnalogSetting(std::string const &line) {
  try {
    CommonSetting const common{std::stoi(GetAttributeValue(line, "x")),
    ...
    int32_t const index{
        Viewer::GetMappingIndex(GetAttributeValue(line, "name"), type_)};
    ...
    if (analog_it == kAnalogDirection.end()) {
      Logger::Error("skin_settings: Invalid analog direction: %s",
                    analog_dir.c_str());
      return false;
    };
    ...
    if (reverse_it == kReverse.end()) {
      Logger::Error("skin_settings: Invalid reverse value: %s",
                    reverse.c_str());
      return false;
    }

    analogs_.push_back(
        AnalogSetting{common, index, analog_it->second, reverse_it->second});
    return true;
...
bool SkinSettings::CreateStickSetting(std::string const &line) {
  try {
    ...
    int32_t const x_index{
        Viewer::GetMappingIndex(GetAttributeValue(line, "xname"), type_)};
    int32_t const y_index{
        Viewer::GetMappingIndex(GetAttributeValue(line, "yname"), type_)};
    ...
    sticks_.push_back(StickSetting{
        ...
        std::stoi(GetAttributeValue(line, "yrange")), x_index, y_index});
    return true;
  } catch (std::exception const & /*unused*/) {
    return false;
  }
...
std::string SkinSettings::GetAttributeValue(std::string const &line,
  ...
  if (attribute_pos == std::string::npos) {
    std::string const err{"Element " + line + " does not contain attribute " +
                          name};
  ...
  if (attribute_start == std::string::npos ||
    ...
    std::string const err{"Element " + line + " is badly formatted"};
    throw std::exception(err.c_str());
  }

  return line.substr(attribute_start + 1, attribute_end - attribute_start - 1);
}
