...
COMDevice::COMDevice(int32_t com_index, int32_t baud_rate,
    ...
    : handle_{nullptr}, com_index_{com_index}, baud_rate_{baud_rate},
      kBufferSize{read_buffer_size}, read_buffer_{new char[kBufferSize]},
      set_data_callback_{set_data_callback}, graphics_update_callback_{
                                                 graphics_update_callback} {
  TryReconnecting();
}

bool COMDevice::Valid() { return handle_ != INVALID_HANDLE_VALUE; }
...
void COMDevice::Tick() {
  try {
    DWORD bytes_read = 0;
    if (!ReadFile(handle_, read_buffer_, static_cast<DWORD>(kBufferSize),
                  &bytes_read, nullptr)) {

      Logger::Error("com_ports: Error reading, trying to reconnect");
      if (!TryReconnecting()) {
        constexpr int32_t kRetyLengthMilli{500};
        Logger::Error("com_ports: Failed to reconnect, trying again in %i",
                      kRetyLengthMilli);
        std::this_thread::sleep_for(
            std::chrono::milliseconds{kRetyLengthMilli});
        return;
      }
    }
...
bool COMDevice::TryReconnecting() {
  ...
  if (!GetCommState(handle_, &serial_params)) {
    ...
    Logger::Error("com_ports: Failed getting parameters for com port %i",
                  com_index_);
    return false;
  }
  ...
  if (!SetCommState(handle_, &serial_params)) {
    handle_ = INVALID_HANDLE_VALUE;
    Logger::Error("com_ports: Failed setting parameters for com port %i",
                  com_index_);
    return false;
  }
...
std::string GetFriendlyName(int32_t device) {
  HDEVINFO h_device_info{
      SetupDiGetClassDevs(&GUID_DEVCLASS_PORTS, 0, nullptr, DIGCF_PRESENT)};
  ...
  if (SetupDiGetDeviceRegistryProperty(
          h_device_info, &dev_info_data, SPDRP_HARDWAREID, &reg_data_type,
          hardware_id.get(), sizeof(hardware_id) * req_size, nullptr)) {
    req_size = 0;
    SetupDiGetDeviceRegistryProperty(h_device_info, &dev_info_data,
                                     SPDRP_FRIENDLYNAME, nullptr, nullptr, 0,
                                     &req_size);

    auto friendly_name{std::make_unique<BYTE[]>((req_size > 1) ? req_size : 1)};
    if (!SetupDiGetDeviceRegistryProperty(
            h_device_info, &dev_info_data, SPDRP_FRIENDLYNAME, nullptr,
            friendly_name.get(), sizeof(friendly_name) * req_size, nullptr)) {
